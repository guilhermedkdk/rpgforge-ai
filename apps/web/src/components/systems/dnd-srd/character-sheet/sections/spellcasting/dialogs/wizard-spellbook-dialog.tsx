...
import { getSpellListLevelLabel, type RuleItemResponse } from '@rpgforce-ai/shared';
import { Dialog, DialogContent, DialogDescription, DialogTitle } from '@/components/ui/dialog';
import { Tooltip, TooltipContent, TooltipTrigger } from '@/components/ui/tooltip';
import { SelectableSpellRow } from '../spell-detail';
...
import type { useWizardSpellbook } from '../hooks/use-wizard-spellbook';
                      ...
                      {levelOpen && (
                        <div className="divide-y divide-border/30">
                          ...
                          {rows.map((spell) => {
                            ...
                            // Free from Evocation Savant: chosen in the feature panel, so here it
                            // only shows checked (no add/remove buttons).
                            const isFromSavant = (spellbook.bySavantByLevel[lvl] ?? []).some(
                              (n) => n.trim().toLowerCase() === keyName
                            );
                            const inSpellbookByAny = isLearned || isFromScroll || isFromSavant;
                            return (
