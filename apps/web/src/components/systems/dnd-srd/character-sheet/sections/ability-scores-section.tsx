...
export function AbilityScoresSection({
  ...
  // Highlight only while setup is actually incomplete (same rule as Skills) — not until the user
  // happens to open the menu; an AI-filled sheet is already complete and must not show the cue.
  const showAttributesSetupCue =
    !locked && (!baseAbilityDistributionComplete || needsBackgroundAbilityBonuses);

  // After a blocked save, the trigger turns red while the distribution is actually incomplete.
  const attributesError =
    pendingFlags.isFlagged('attributes') &&
    (!baseAbilityDistributionComplete || needsBackgroundAbilityBonuses);
              ...
              <DropdownMenu>
                <DropdownMenuTrigger asChild>
                  <button
                    ...
                    onPointerDown={() => pendingFlags.dismiss('attributes')}
                  >
                    Attributes
                  </button>
                </DropdownMenuTrigger>
                      ...
                      <div className="rounded-lg border border-border bg-muted/30 p-3">
                        {abilityMethod === 'standard-array' ? (
                          <p className="text-xs text-muted-foreground">
                            You have six fixed scores to assign across your six abilities: 15, 14,
                            13, 12, 10, and 8. Each ability takes exactly one of those numbers, and
                            each number is used only once. Open the menu under each score until
                            every value is placed.
                          </p>
                        ) : (
                          <p className="text-xs text-muted-foreground">
                            You have 27 points to spend across your six abilities. Each one starts
                            at 8, and you can raise any score up to 15. Raising a score costs more
                            points the higher it becomes. Use the +/− controls until you have spent
                            your full budget.
                          </p>
                        )}
                      </div>
                          ...
                          {data.backgroundAbilityScoreOption ? (
                            <>
                              <p className="mb-3 text-xs text-muted-foreground">
                                ...
                                {data.backgroundAbilityScoreOption.totalPoints} point
                              ...
                              {!baseAbilityDistributionComplete ? (
                                ...
                                >
                                  ...
                                  <p className="mt-1 text-destructive/90">
                                    Finish your six base ability scores (Point Buy or Standard
                                    Array) before spending background bonus points.
                                  </p>
                                </div>
                              ) : null}
                              <div className="rounded-lg border border-border bg-muted/30 p-3">
                                <div className="space-y-2">
                                  {(data.backgroundAbilityScoreOption.allowedAbilityNames.length > 0
                                    ...
                                    const current =
                                      data.backgroundAbilityScoreIncrease?.[attr] ?? 0;
