...
import { isSpellcastingTableFeature } from './spellcasting-table';
import { isInlineColumnTableFeature } from './feature-column-table';
import {
  DeftExplorerExpertisePickerBlock,
  DeftExplorerLanguagesPickerBlock,
} from '../panels/deft-explorer';
import { ThievesCantLanguagePickerBlock } from '../panels/thieves-cant';
import type { FeatureDetail } from '../shared/types';
import { SELECTION_INLINE_TOP_RULE } from '../shared/selection';
import {
  FeatureOptionRow,
...
const SKIP_INLINE_OPTION_NAMES = new Set([
  ...
  // Subclass: name-only options rendered by SubclassTypeChoicePanel (the inline fallback
  // would add an empty selection column to the Circle of the Land spell tables).
  'elemental affinity',
  'fiendish resilience',
...
export function GenericOptionsView({
  ...
  // Key first, display name only as a fallback (same rule the rest of the sheet follows).
  const featureAsMechanics = { name: featureName, featureKey: feat.featureKey };
  ...
  if (
    ...
    !isThievesCantFeature(featureAsMechanics)
  ) {
  ...
  // Subclass with name-only options: the choice sits right after the intro text and BEFORE the
  // tables (Circle of the Land Spells), since it determines which table applies.
  if (feat.source === 'subclass' && isTypeChoiceSubclassFeatureName(featureName)) {
    const segmentClass = '[&_p]:mb-1.5 [&_p:last-child]:mb-0';
    const split = splitDescAtFirstTable(desc);
    const intro = split ? split.intro : desc;
    return (
      <>
        {intro.trim().length > 0 ? (
          <div className={segmentClass}>
        ...
        <SubclassTypeChoicePanel feat={feat} data={data} onChange={onChange} />
        {split ? (
          <div className={cn(segmentClass, 'mt-4')}>
            <ReactMarkdown remarkPlugins={[remarkGfm]}>{split.tables}</ReactMarkdown>
          </div>
        ) : null}
      </>
    );
  }
  if (isThievesCantFeature(featureAsMechanics)) {
    const segmentClass = '[&_p]:mb-1.5 [&_p:last-child]:mb-0';
    return (
      <>
        {desc.trim().length > 0 ? (
