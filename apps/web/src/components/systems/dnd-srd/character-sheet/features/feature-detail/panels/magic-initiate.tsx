...
export function MagicInitiatePanel({
  ...
  const toggleSpell1 = (name: string) => {
    const isSame = selectedSpell?.toLowerCase() === name.toLowerCase();
    // Limit is 1: deselect the current pick to free the slot before choosing another
    // (no auto-swap — matches the project's selection standard, same as cantrips).
    if (isSame) patchGain({ spellName: null });
    else if (!selectedSpell) patchGain({ spellName: name });
  };
