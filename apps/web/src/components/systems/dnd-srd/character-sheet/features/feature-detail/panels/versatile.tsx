...
export function VersatilePanel({ data, onChange, featsList }: VersatilePanelProps) {
  ...
  return (
    <SelectionSection>
      ...
      <p className="mb-3 text-xs text-muted-foreground">
        Feats you already have are shown as pre-filled and locked. Only Magic Initiate and Skilled
        can be selected more than once.
      </p>
      <div
        className="max-h-80 space-y-2 overflow-y-auto pb-2 pr-1"
        role="list"
        aria-label="Origin feats"
      >
        {originFeats.map((feat) => {
          ...
          const isAlreadyOwnedNonRepeatable =
            !isRepeatableFeat && ownedFeatIdsSet.has(feat.id) && feat.id !== selectedFeatIdForSlot;
