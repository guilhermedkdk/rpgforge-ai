...
/** A feat's SRD category, read from its normalized/raw `type`. */
export type FeatCategory = 'origin' | 'general' | 'fighting-style' | 'epic-boon' | 'other';

/**
 * Single source for "what category is this feat" (Origin / General / Fighting Style / Epic Boon),
 * so the per-feat pickers (ASI, Versatile, Epic Boon, Fighting Style, Eldritch Invocations) and the
 * AI generation menus never disagree. Returns 'other' when the type is missing/unknown.
 */
export function getFeatCategory(feat: RuleItemResponse): FeatCategory {
  const type = String(
...
export const isEpicBoonFeat = (feat: RuleItemResponse): boolean =>
  getFeatCategory(feat) === 'epic-boon';
...
export function reconcileFeatPrerequisites(
  ...
  if (prereqUnmet(data.additionalFightingStyleFeatId)) patch.additionalFightingStyleFeatId = null;
