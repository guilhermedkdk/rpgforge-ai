...
describe('getEffectiveAttribute', () => {
  ...
  it('adds background increase and Epic Boon (+1 to the chosen ability)', () => {
    ...
    expect(getEffectiveAttribute(attrs({ Charisma: 17 }), undefined, 'Charisma', 'Charisma')).toBe(
      18
    );
  });
  ...
  it('applies Primal Champion (+4 Str/Con, cap 25) and Body and Mind (+4 Dex/Wis, cap 25)', () => {
    expect(
      getEffectiveAttribute(attrs({ Strength: 18 }), undefined, 'Strength', null, true, false)
    ).toBe(22);
    // cap at 25
    expect(
      getEffectiveAttribute(
        attrs({ Constitution: 24 }),
        undefined,
        'Constitution',
        null,
        true,
    ...
    expect(
      getEffectiveAttribute(attrs({ Wisdom: 20 }), undefined, 'Wisdom', null, false, true)
    ).toBe(24);
    // Primal Champion does not touch Dex
    expect(
      getEffectiveAttribute(attrs({ Dexterity: 18 }), undefined, 'Dexterity', null, true, false)
    ).toBe(18);
  });
  ...
  it('caps Epic Boon at 30 and never reduces', () => {
    expect(getEffectiveAttribute(attrs({ Strength: 29 }), undefined, 'Strength', 'Strength')).toBe(
      30
    );
    expect(getEffectiveAttribute(attrs({ Strength: 30 }), undefined, 'Strength', 'Strength')).toBe(
      30
    );
  });
